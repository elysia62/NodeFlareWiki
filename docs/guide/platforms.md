...
安装脚本自动检测系统服务管理器（systemd / OpenRC / launchd / FreeBSD rc / Windows 计划任务）并注册为开机自启；Docker 部署依赖 `restart unless-stopped`，见 [Docker 部署](/guide/docker)。
...
| Docker | 镜像内已包含 | 容器内 `/etc/nodeflare`，示例挂载宿主机的同名目录 |

Agent（Linux）：程序位于 `/opt/nodeflare/agent`，配置与状态位于 `/etc/nodeflare/agent`。SQLite 文件位于配置目录下。

按示例部署 Docker 时，宿主机与容器内的配置文件路径均为 `/etc/nodeflare/config.toml`，目录及配置文件需允许容器用户 `10001:10001` 读写，见 [Docker 部署](/guide/docker#准备配置)。
