...
官方镜像 `gxmandppx/nodeflare`（[Docker Hub](https://hub.docker.com/r/gxmandppx/nodeflare)）支持 amd64 / arm64。示例以 Linux 宿主机为例，将 `/etc/nodeflare` 挂载到容器内的同一路径，持久保存配置与数据。

## 首次部署

### 准备配置

首次部署时，在宿主机创建 `/etc/nodeflare`，下载配置示例保存为 `config.toml`。容器启动时会读取该文件：
...
编辑 `/etc/nodeflare/config.toml`，至少填好管理员账号密码：
...
其余保持默认。`bind_addr = "0.0.0.0:2206"` 是容器内的监听地址，**不要改成 `127.0.0.1`**，否则端口映射后无法访问。

::: tip 目录权限
编辑完成后，将目录与配置文件所有者设为容器用户 `10001:10001`：
...
升级前建议先在后台「数据库」页面导出 ZIP 备份并保存到本地，见[数据库与备份](/guide/database)。重建容器时继续使用原有挂载目录；既有部署若使用 `./data` 或其他目录，保留原路径即可，无需迁移到 `/etc/nodeflare`。
...
宿主机挂载目录中的配置、默认 SQLite 数据库和主题会保留。更新时继续使用现有 `config.toml`；后台导出的 ZIP 备份由浏览器下载，不会自动保存到该目录。

默认使用 `latest` 标签。需固定版本时，将 `latest` 替换为 Docker Hub 上已发布的版本标签（不带 `v`）；升级时先修改镜像标签，再拉取并重建容器。
...
需要备份时，先在容器运行期间从后台「数据库」页面导出并保存到本地，见[数据库与备份](/guide/database)。随后按部署方式选择命令停止并删除容器：
...
卸载后宿主机的 `/etc/nodeflare` 目录仍会保留；使用其他挂载目录时以实际路径为准。

::: warning
该目录包含配置、默认 SQLite 数据库和主题；同机安装 Agent 时还可能包含 `agent/`。删除前请确认备份已保存，且目录中的文件都不再需要。
:::
