  ...
  return (
    <form className="settings-form" onSubmit={tab === "data" ? (event) => { event.preventDefault(); void migrateDatabase(); } : saveSite}>
      ...
      {tab === "themeSettings" ? <>
        ...
        <p className="settings-hint">{ui(locale, "仅支持 HTTPS；浅色与深色背景用 | 分隔。", "HTTPS only; separate light and dark backgrounds with |.")}</p>
      ...
      {tab === "alerts" ? <>
        ...
        <p className="settings-hint">{ui(locale, "达到阈值后，每超过 5 个百分点提醒一次，上限 100%。", "After the threshold is reached, alerts fire for every additional 5 percentage points, up to 100%.")}</p>
      ...
      {tab === "security" ? <>
        ...
        <div className="two-factor-panel">
          ...
          {twoFactorSetup ? <div className="two-factor-setup">
            ...
            <p className="settings-hint">{ui(locale, "在 Google Authenticator、Aegis、2FAS 等验证器中手动输入密钥，再填入当前 6 位验证码确认。", "Enter the secret in an authenticator such as Google Authenticator, Aegis or 2FAS, then confirm with the current 6-digit code.")}</p>
          </div> : null}
          {twoFactorStatus?.enabled ? <div className="two-factor-actions"><button type="button" className="danger-btn two-factor-disable-btn" disabled={busy} onClick={() => void disableTwoFactor()}>{ui(locale, "禁用两步验证", "Disable two-factor authentication")}</button></div> : <div className="two-factor-actions">
            {!twoFactorSetup ? <p className="settings-hint">{twoFactorStatus?.has_secret ? ui(locale, "已有未启用的密钥；重新生成会使旧密钥失效。", "A disabled secret exists; regenerating invalidates the old one.") : ui(locale, "尚未生成两步验证密钥。", "No two-factor secret has been generated yet.")}</p> : null}
        ...
        <div className="login-devices-panel">
          <div className="login-devices-head"><div><div className="section-subtitle">{ui(locale, "登录设备", "Signed-in devices")}</div><p className="settings-hint">{ui(locale, "下线后，该设备的登录状态立即失效。", "Signing out a device invalidates its session immediately.")}</p></div><button type="button" className="secondary-btn compact" disabled={Boolean(revokingSessionId)} onClick={() => void loadLoginSessions()}><RotateCw size={14} />{ui(locale, "刷新", "Refresh")}</button></div>
